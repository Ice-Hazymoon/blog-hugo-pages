...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"vqlN":[function(require,module,exports) {
/**!
...
document.addEventListener('swup:contentReplaced', async () => {
  await initSearch();
});
initSearch().then(() => {
...
async function initSearch() {
  ...
  let data;
  const searchIndexCache = sessionStorage.getItem('searchIndex');

  if (searchIndexCache) {
    data = JSON.parse(searchIndexCache);
  } else {
    try {
      data = await fetch('/search/index.json').then(e => e.json());
      sessionStorage.setItem('searchIndex', JSON.stringify(data));
    } catch (error) {
      alert('初始化搜索引擎失败');
      console.log(err);
    }
  ...
  const index = new _flexsearch.default.Index({
    cache: true,
    tokenize: "full"
  });
  data.forEach((item, i) => {
    index.add(i, item.content);
  });
  let cplock = false;
  ...
  function render(value) {
    ...
    if (value) {
      const html = `<div class="p-6 pb-0">
                <div class=" font-bold text-2xl border-b pb-4">
                    关键字
                    <span class="text-theme">${value}</span>
                    中共找到
                    <span class=" text-theme">${results.length}</span>
                    条结果
                </div>
                ` + results.map(n => {
        ...
        if (n.content.match(regex)) {
          return `<a
                            href="${n.permalink}"
                            class="border-b py-4 block search-results-items"
                        >
                            <div class=" text-2xl mb-2 line-clamp-1 ts-3-ease">
                                ${n.title}
                            </div>
                            <div class=" mb-2 line-clamp-2">` + n.content.match(regex)[0].replace(new RegExp(value, 'mi'), x => {
